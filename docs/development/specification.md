...
- **D27 `all_solvers()` and the two halves of the test battery.** `skroute.all_solvers() -> list[type[BaseRouter]]` returns, sorted by `__name__`, every concrete solver class exported from `skroute` that can be instantiated with no arguments: the 15 (16) solvers of D18 plus `EnsembleGenetic` and `EnsembleSimulatedAnnealing`; `MultiStart` is never returned (it needs an estimator) and is covered by `tests/test_ensemble.py`. `check_router(estimator)` (an **unfitted instance**) runs only the structural checks 1–11, 13 and 14 of §6 on instances it builds itself; the tolerance checks live in `tests/test_common.py` and are driven by `tests/tolerances.py`, the single source of every tolerance number.
...
- **D30 Progress callbacks and live visualisation (lead, 2026-09-04).** Every solver accepts `fit(..., callback=None)`. `callback` is any callable taking one `skroute.base.RouteEvent` and returning `None` or a bool; returning `True` asks the solver to stop after the current outer iteration (`stop_reason_ = "callback"`, added to the allowed subset of every iterative solver). `RouteEvent` is a frozen dataclass: `solver` (class name), `stage` (`"start"`, `"iteration"`, `"end"`), `iteration` (outer iteration index, 0 at start), `cost` (objective of `tour` as the solver knows it, `nan` when unknown), `best_cost`, `tour` (label-space open giant tour of the CURRENT solution, depot first, or `None` when the solver has no tour yet — MILP before its first integral solution), `best_tour` (label-space best-so-far tour), `problem` (the `RoutingProblem`), `extra` (a dict of solver-specific facts: `temperature`, `tenure`, `generation`, `n_evaluations`, `kick`, `moves_applied`, `edges` for MILP's current support, `restart` for MultiStart), plus the properties `route` and `trips` (decoded from `best_tour` with the problem's split rule). The base class implements `_emit(stage, iteration, tour_idx, cost, best_tour_idx, best_cost, **extra)` — a no-op when no callback is set, so the fit path stays free of overhead — and the flag `_stop_requested`; every iterative solver emits `start` once, `iteration` once per outer iteration (where it updates `history_`) and `end` once; construction and exact solvers emit `start` and `end` (MILP also one `iteration` per cut round with `extra["edges"]`; since D31 the construction solvers also emit one `iteration` per construction step with `tour=None`, `cost=nan` and `extra["edges"]`); `MultiStart` forwards the callback to the inner estimators only when `n_jobs` is `None` or `1` (matplotlib is not thread-safe; with a callback the sequential path also ignores an enclosing `joblib.parallel_config`) and emits its own `start`/`end`. `check_router` gains check 14: the event sequence is well formed, every non-`None` tour is a valid label permutation, `best_cost` is non-increasing, and returning `True` from the callback stops an iterative solver with `stop_reason_ == "callback"`. The package `skroute.viz` (extra `viz`: `matplotlib>=3.8`; `plotly>=5.24` optional for maps) offers `plot_route(...)` (points and the closed tour, one colour per trip), `plot_history(...)`, `LivePlot(coords, backend=None, map=False, every=1)` (`backend` resolves to `"matplotlib"`, or to `"plotly"` when `map=True`; `"plotly"` may also be asked for explicitly) — a callback that redraws the current and best tours while `fit` runs, in scripts (interactive matplotlib) and notebooks (Plotly `FigureWidget` or `IPython.display`) — and `Recorder()` — a callback that keeps every event and produces a matplotlib `FuncAnimation` (`.animate(coords)`, savable as GIF) or a Plotly figure with a slider (`.to_plotly(coords, map=...)`); `map=True` draws on OpenStreetMap tiles through Plotly's `Scattermap` for `(lat, lon)` coordinates. Nothing in `skroute.viz` is imported by the rest of the package.
- **D31 Traces of partial structures and time-lapse replay (lead, 2026-09-04).** The owner wants to watch the route *being built* and to replay a run at high speed, for every solver family. Three optional `extra` keys are standardised on `RouteEvent` (viewers ignore keys they do not know): `extra["edges"]` — a list of `(label, label)` pairs describing the partial structure the solver holds when `tour` is `None` or incomplete (the growing path of NearestNeighbour, the partial cycle of Insertion after each insertion, the current trips of ClarkeWright after each merge, the edge set of NRBS after each connection, the LP support of MILP); `extra["edge_weights"]` — a list of floats in `[0, 1]` parallel to `extra["edges"]` (AntColony's pheromone strength of its strongest `3n` trails after each iteration); `extra["ring"]` — an `(m, 2)` float array with the positions of SOM's neurons in the units of `problem.coords` after each epoch. Construction solvers emit one `iteration` event per construction step with `tour=None`, `cost=nan`, `best_cost=nan` and `extra["edges"]` (replayed from the kernel's recorded insertion order when the step happens inside Cython; this costs nothing without a callback); `check_router` treats `nan` costs as unknown (excluded from the monotonicity check) and requires that `extra["edges"]` pairs are labels of the problem. `Recorder` stores `extra` and a wall-clock timestamp per event; `Recorder.animate(coords, show="both"|"best"|"current", speed=..., fps=...)` draws the current tour thin and the best tour thick (plus edges, weights and ring when present), `Recorder.replay(coords, speed=...)` drives a `LivePlot` at time-lapse speed respecting the recorded timings, `Recorder.save(path, fps=..)` writes GIF (pillow) or MP4 (ffmpeg when available), and `Recorder.to_plotly` offers Play/Pause and a speed menu. `LivePlot` draws the same three extras live. The README GIF shows the attempts and the best tour of a metaheuristic; the user guide shows a construction heuristic growing node by node and SOM's ring closing on the cities.
- **D32 Service time (lead, 2026-09-05).** A visit takes time. `RoutingProblem(..., service_time=None)`, `fit(..., service_time=None)` and `route_cost(..., service_time=None)` accept a scalar or an `(n,)` array-like in the units of `time_matrix` (finite, `>= 0`; requires `max_time_work`, like `time_matrix`; a scalar applies to every non-depot node). The kernels receive the *effective* time matrix `T_eff[i, j] = T[i, j] + service[j]` for `j != depot` and `T_eff[i, depot] = T[i, depot]`, plus `T_eff[depot, j] += service[depot]` (a service at the depot is paid once per trip, at departure); `problem.time` stays the raw travel-time matrix, `problem.service_time` is the `(n,)` float64 array (all zeros when not given), `problem.time_or_cost` returns `T_eff`, and `trip_times` therefore include the services. The feasibility check of the depot round trips uses `T_eff` (message names the node and the service). `check_router` check 8 (multi-trip) gains one instance with `service_time`; `fit_time_` unaffected. `skroute.metrics.timetable(obj, route=None, *, start="08:00", units="min", as_frame=False) -> list[list[Stop]]` turns a fitted estimator (or a problem and a label-space route) into a per-trip timetable of frozen `Stop(day, order, label, arrival, departure, travel, service, wait=0.0, start=480.0)` records — minutes since `start`, with `Stop.arrival_time`/`departure_time` strings `HH:MM` — computed from `problem.time` and `problem.service_time` (the depot appears as the first and last stop of every day: at order 0 `arrival = 0` and `departure = service[depot]`, i.e. the vehicle leaves at `start` plus the depot's service; at the return `arrival` is the end of the day); pure Python, pandas optional through `as_frame=True`; `timetable_summary(days)` totals every day. The final signatures and rules are in §3.3 'Service time'.
- **D33 Real-world inputs from map services (lead, 2026-09-05).** New module `skroute.preprocessing.maps` (re-exported from `skroute.preprocessing`), standard library only (`urllib.request`, `json`), every function honouring a `timeout=` and retrying a failed HTTP call up to 3 times with back-off: `travel_time_matrix(coords, *, provider="osrm", mode="driving", units="min", api_key=None, departure_time=None, base_url=None, chunk_size=None, user_agent=None) -> Bunch(time, distance, units, provider, coords)` — `coords` are `(lat, lon)` pairs; the matrices are square, float64, asymmetric when the roads are, in minutes (`units["time"]`, also `"s"`/`"h"`) and metres (`units["distance"]`); provider `"osrm"` calls the table service of an OSRM server (default the public demo `https://router.project-osrm.org`, whose limits are respected by tiling the table in `chunk_size x chunk_size` blocks with `sources`/`destinations`, default 50, and by pausing between requests), provider `"google"` delegates to `GoogleDistanceMatrix` (requires the key and the `google` extra; `departure_time="now"` or a datetime asks for `duration_in_traffic`), unroutable pairs become `nan` with one WARNING. `geocode(query, *, provider="nominatim", api_key=None, user_agent=None, timeout=10) -> Bunch(lat, lon, display_name, raw)` — Nominatim needs a descriptive `User-Agent` (default `scikit-route/<version> (+https://github.com/arubiales/scikit-route)`) and at most one request per second (enforced); provider `"google"` uses the Geocoding API. `fetch_pois(area, *, brand=None, name=None, amenity=None, wikidata=None, provider="overpass", base_url=None, timeout=90) -> Bunch(coords, labels, names, addresses, tags, DESCR)` — one Overpass QL query (`GET ?data=` — the public server answers 406 to form POSTs) selecting an administrative area by name (`area["boundary"="administrative"]["name"=...]`) and `nwr` elements matching the filters, using `out center` so ways and relations become points; labels are the OSM ids (`node/123`), `DESCR` carries the ODbL attribution ("© OpenStreetMap contributors"). Tests never touch the network: recorded JSON answers under `tests/data/maps/` are served through a monkeypatched opener; a `network` marker (deselected by default, run by the nightly) exercises the live services on three points.
- **D34 Schedules on Google Maps (lead, 2026-09-05).** New module `skroute.viz.google_maps` (importable without matplotlib; nothing here needs an API key except the JavaScript page): `google_maps_urls(route_or_estimator, coords, *, mode="driving", max_waypoints=9) -> list[list[str]]` — one Google Maps Directions URL per trip (`https://www.google.com/maps/dir/?api=1&origin=...&destination=...&waypoints=...&travelmode=driving`, coordinates as `lat,lon` with 6 decimals), a trip longer than `max_waypoints` intermediate stops split into consecutive legs that share their end/start point (Google's documented limit for the URL scheme); `to_kml(route_or_estimator, coords, path, *, names=None, depot_name="Depot", trip_names=None, colors=None) -> Path` — a KML 2.2 document with one `Folder` per trip holding numbered `Placemark`s (name, description with the order and the day) and a `LineString` of the closed trip, distinct line colours per trip, importable into Google My Maps and Google Earth (the docstring says how: My Maps → Create → Import); `google_maps_html(route_or_estimator, coords, path, *, api_key=None, names=None, trip_names=None, title=None) -> Path` — a standalone HTML page loading the Maps JavaScript API with `api_key` (or `GOOGLE_MAPS_API_KEY` from the environment; `ValueError` naming the variable when both are missing) that draws every trip with a `DirectionsService`/`DirectionsRenderer` (requests of at most 25 waypoints, split like the URLs), numbered markers, one colour per trip, a legend with checkboxes toggling the days and the totals (stops, driving minutes) computed in Python and embedded as JSON; the key-free interactive alternative stays `plot_route_map` (Plotly on OpenStreetMap tiles), which gains `names=` hover labels and `trip_names=`. Both modules are covered by tests that parse the produced URLs, KML (with `xml.etree`) and HTML (the JSON block) — no browser, no network.
- **D35 Worked case: a maintenance technician's route plan (lead, 2026-09-05).** `examples/technician_madrid.py` solves the owner's case: every Burger King restaurant of the Comunidad de Madrid (from OpenStreetMap through `fetch_pois`, brand:wikidata Q177054), the office at Calle Ramón y Cajal 18, Leganés (through `geocode`), real driving times (`travel_time_matrix`, OSRM by default, Google with `--google-key`/`GOOGLE_MAPS_API_KEY`), 30 minutes of service per restaurant (`--service`), 8-hour days (`--hours`) and one technician; the objective is total driving time with `extra_cost=` one day's budget per extra day (fewest days first, then least driving), `split="optimal"`, solved with `MultiStart(IteratedLocalSearch)` (`--solver`, `--time-limit`), and reported as: a per-day timetable on the console, `technician_madrid_timetable.csv`, the KML, the Google Maps URLs, the Google Maps page when a key is given, the Plotly map (`technician_madrid_map.html`) and a PNG of the days. The data of the run (restaurants with names and coordinates, the office, and the OSRM matrix) are committed as CSV under `examples/data/` with their attribution so the example is reproducible offline (`--refresh` fetches live data); `docs/user_guide/real_world.md` narrates the case with the resulting table and pictures, and states the assumptions (static travel times, no lunch break, every restaurant open when visited). Version 2.1.0.
...
scikit-route/
  ...
  skroute/
    ...
    metrics.py                   [L] route_cost, split_trips, timetable, Stop, timetable_summary (public recomputation helpers; D32)
...
- a real scalar (`_is_number`: never a bool; a 0-d numeric ndarray is unwrapped to the scalar it holds first) -> `np.full(n, float(value))` with `s[depot] = 0.0`; rejected unless finite and `>= 0`: `ValueError("service_time must be a finite number >= 0, got {value!r}")`. The test is pure Python (`problem._finite`: `float(value)` then `math.isfinite`, `None` on any failure), so a `Fraction` or an int beyond the float range get this message and never numpy's `TypeError`; the same helper validates `max_time_work` and `extra_cost`;
- a bool, a str/bytes, a `Decimal`, a dict, a complex, a list of strings — anything that is neither a real scalar nor something numpy reads as a float array — -> `ValueError("service_time must be a number or an ({n},) array-like, got {type(value).__name__}")` (a type-oriented message, never "got shape ()");
- anything else -> `np.array(value, dtype=np.float64)` (a copy: the caller's array is never aliased), which must have shape `(n,)` (`"service_time must be a scalar or have shape ({n},), got shape {shape}"`), be finite (`"service_time contains NaN or infinite values"`) and non-negative (`"service_time contains negative durations"`). Labels are not read: the array is in matrix row order, like `coords` — with one guard: a pandas Series (duck-typed, `index` and `to_numpy`) must carry the labels of `X` in row order, `ValueError("service_time index differs from the labels of X")` otherwise (checked after the shape, like `time_matrix`'s labels; nothing is reordered — a Series in another order would silently move every service, the depot's zero included).
...
Aliasing (decided in review round 5): `RoutingProblem` never writes to its arrays, but `coerce_matrix` does not copy a float64 C-contiguous input — `cost` and `time` are then *views* of the caller's arrays (no second n×n matrix at n = 10 639, D13) — whereas `service_time` is always copied and `_time_eff` is a private matrix whenever a service is non-zero. An array must therefore not be mutated after it has been passed: a later write to the caller's `time_matrix` would move `timetable` (which reads `time`) but not `trip_times_` (which reads `_time_eff`). The class docstring says so; "immutable" means the problem itself never changes.

`skroute.metrics` (D32): `timetable(obj, route=None, *, start="08:00", units="min", as_frame=False) -> list[list[Stop]]` — `obj` a fitted estimator (`problem_` and `route_`; `route` may override the route) or a `RoutingProblem` (then `route` is required); a route with the depot repeated in its interior is read **as driven** (one day per segment, every non-depot label exactly once — whatever `max_time_work` says: a day that ends after the budget is kept and reported with `UserWarning("day {k} ends at {end:g} min, over max_time_work ({budget:g} min): the route was read as driven, ...")`, never re-cut; tolerance `1e-9 * max(1, budget)` minutes, so a decoded day never warns), otherwise it is a giant tour (open or closed) decoded with `problem.trip_starts`. Note the asymmetry with `route_cost`, which always removes the depot occurrences and re-decodes the giant tour: on a hand-made plan the two describe different plans (the timetable the user's days, `route_cost` the decoder's cut), and both docstrings and `multi_trip.md` say so; they agree on every route `fit` produces. `ValueError("timetable needs time_matrix and max_time_work")` for a plain TSP, `TypeError` for anything else, `NotFittedError` for an unfitted estimator. `start` is `"HH:MM"` or a `datetime.time` (seconds and microseconds count) — the start of the day at the depot: the vehicle leaves at `start + service[depot]`; `units` (`"min"`, `"h"`, `"s"`) are the units of `problem.time` — the timetable is always in minutes (added to the D32 signature so that the hours tables of `skroute.datasets` get correct clock strings). `Stop` is a frozen dataclass `Stop(day, order, label, arrival, departure, travel, service, wait=0.0, start=480.0)`: `day` 1-based, `order` 0 at the departure from the depot and last at the return, `arrival`/`departure`/`travel`/`service` in minutes since `start` (`departure = arrival + service + wait`; at the departure stop `arrival = 0`, `departure = service[depot]`; at the return `service = 0`, `departure = arrival`), `wait` always `0.0` today (reserved for time windows), `start` the minutes after midnight of the day's start (excluded from the repr **and from equality and hashing** — `field(repr=False, compare=False)` — so stops with the same minutes are equal whatever clock they are rendered on); properties `arrival_time`/`departure_time` -> `HH:MM` rounded to the nearest minute, wrapping past midnight. `as_frame=True` returns a `pandas.DataFrame` with columns `day, order, label, arrival_time, departure_time, travel, service, wait, arrival, departure` (pandas imported lazily; the `ImportError` names `scikit-route[pandas]`). `timetable_summary(days) -> list[dict]` gives per day `day, n_stops, driving, service, total, back_at`; it takes the list form only — `TypeError` for anything else, the DataFrame of `as_frame=True` included (the message says to group it by `day`). `timetable` carries three `@overload`s (`as_frame: Literal[False]` -> `list[list[Stop]]`, `Literal[True]` -> `pandas.DataFrame`, `bool` -> the union) so the common path is typed, not `Any`. The end of every day equals the corresponding `trip_times_` value (converted to minutes), which is how the hypothesis test of `tests/test_timetable.py` proves that every day of a fitted solution fits the budget.
...
6. Invalid inputs raise `ValueError` with the messages of §3.3 (non-square, NaN, unknown depot, `max_time_work` without `time_matrix`, `time_matrix` without `max_time_work`, RoutingProblem plus kwargs, and the D32 ones: `service_time` without `max_time_work`, a negative scalar, a wrong shape, a negative entry — so a `fit` override that swallows or reorders `service_time` is caught); infeasible node → `InfeasibleProblemError`.
...
8. Multi-trip (alicante; skipped for solvers with `exact and not budget_aware`, which raise under a budget per D6 and check 7; non-exact budget-unaware solvers run it under the `UserWarning`): `trip_times_ <= budget + 1e-9`, `trip_times_` absent for plain TSP; **same tour, both decoders**: after a fit under `split="greedy"`, `p_opt = RoutingProblem(d.cost, time_matrix=d.time, split="optimal", **kw)` and `p_opt.evaluate(est.problem_.to_index_tour(est.tour_)) <= est.cost_ + 1e-9`. Comparing two independent heuristic fits (`fit(optimal).cost_ <= fit(greedy).cost_`) is NOT asserted — different searches return different tours; that inequality is asserted only for exact budget-aware solvers (BruteForce) in `test_exact.py`. **D32**: a scalar `service_time = budget / 8` coerces to the array of §3.3 (0 at the depot; no fit needed), and a fit with the per-node array `s` (`budget / 8` at every customer, `budget / 16` at the depot, so the once-per-trip departure term is exercised) has `problem_.service_time == s` (float64, copied), `problem_.time` raw, `trip_times_ <= budget + 1e-9` with each value equal to the trip's driving time plus the services of its nodes plus `s[depot]`, `problem_.trip_times` reproducing `trip_times_`, and equals — same `tour_`, `trips_`, `cost_`, `trip_times_` — the fit on the time matrix with the services folded in by hand (`folded[:, others] += budget / 8`, `folded[depot, others] += s[depot]`, diagonal raw). A solver that prices durations on `problem.time` fails here (`tests/test_utils.py`).
...
14. **Callback protocol (D30, D31)**: `fit(callback=<non-callable>)` raises `TypeError` naming the argument; with a recording callback the events form `start, iteration*, end` per emitting solver (`iteration` indexed `1, 2, ...`, strictly increasing; one per entry of `history_` for an iterative solver), every non-`None` tour is a valid label tour priced exactly by its `cost`/`best_cost`, a missing tour has a `nan` cost — unknown, excluded from the monotonicity check — and `best_cost` is non-increasing over the finite values and ends at `cost_` with `tour_` (`route`/`trips` decoding to `route_`/`trips_`); the D31 extras are well formed when present (`extra["edges"]` a list of `(label, label)` tuples of labels of the problem, `extra["edge_weights"]` a parallel list of numbers in `[0, 1]`, `extra["ring"]` a finite float `(m, 2)` array); the same trace holds under string labels with a non-first depot (event tours are object arrays starting at the depot label); recording does not change the result; neither the callback nor a stop request survives the fit; returning `True` stops an iterative solver after one outer iteration with `stop_reason_ == "callback"` and silences the trace of a non-iterative one (at most one more `iteration` event before `end`; a construction solver's result is unchanged).
...
- 2026-09-03 Review round 1 (foundation). Spine: `RoutingProblem.__repr__` prints the plain depot label (`.item()`); `neighbours()` breaks ties at the k-th distance by node index (block-wise partition with tie detection); `RoutingProblem` copies its matrices (superseded in review round 5: a float64 C-contiguous input is kept as a view, see §3.3 "Aliasing") (immutable, never aliases the caller's arrays) and validates `tour`/`starts` buffers before handing them to the boundscheck-free kernels; the sub-modules `exceptions`, `metrics`, `utils`, `datasets`, `preprocessing`, `base`, `problem` are reachable after a bare `import skroute`; `check_router` enforces the `stop_reason_` subsets by class name (`_ALLOWED_STOP_REASONS`) and uses `capfd`; check 11 escalates to n = 80 asymmetric and n = 150 instances before declaring two seeds indistinguishable (IteratedLocalSearch and AntColony reach the same optimum with every seed at n <= 40). Core: the greedy decoder never reads the diagonal (the oracle mirrors it), depot-only tours are rejected, `or_opt_descent` prices and resets from the real anchor and scans from both segment ends and from the anchor, `problem_cost_py`/`trip_starts` validate and allocate with the GIL held and release it around the kernel. Data: `optimal_tour_length` of kz9976 is 1061881 (proven) and of ch71009 4566506 (best known; bm33708 and ch71009 are presented as open problems); the 27 country wrappers accept a positional `mode` for 1.0 compatibility; `pairs_to_matrix` rejects conflicting duplicate pairs and keeps mixed-type labels through `coerce_labels`; the TSPLIB reader tolerates keyword lines inside a section and a UTF-8 BOM.
- 2026-09-03 Wave B (solvers), recorded deviations: kernels of HeldKarp and Insertion are `_hk.pyx` and `_insert.pyx` (a `.py`/`.pyx` pair of the same module name lets the `.so` shadow the `.py`); BruteForce prices the reversed orientation when the kept one is within 1e-9 of the incumbent so ties equal `itertools.permutations` bit for bit; `MILP.lower_bound_` starts from a trivial valid assignment bound, is clipped to `cost_`, uses `mip_dual_bound` when `mip_rel_gap > 0`, accepts `time_limit=None`, and its ATSP time-out fallback polishes with `local_search_generic`; `Insertion(strategy="cheapest")` is O(n^3) worst case; NRBS divides by `max(C[i, j], 1e-12)^f` with `i` the node; TwoOpt TINY tolerance is 0.11; convergence of the descents requires a zero-gain full sweep with every don't-look bit active; `IteratedLocalSearch.n_iter_` counts kick iterations only, `temperature=None` is 0.5 % of the init tour cost, `local_search=None` is allowed (kicks only); the deterministic descents reject `init="random"` at validation; TabuSearch's symmetric path also evaluates no-reversal Or-opt relocations and excludes the full-reversal 2-opt move, applies the best move overall when every move is tabu; SA `moves` accepts a list/tuple/str; the fitter tournament winner is Genetic's parent 1; `improvement(new, inf)` accepts the first solution; AntColony floors zero off-diagonal costs and uses its own `n_candidates` for the polish; extra attributes `Genetic.n_duplicates_`, `AntColony.pheromone_`, `SOM.n_samples_`; SOM initialises weights uniformly inside the bounding box; MultiStart copies the inner best estimator's `stop_reason_` (check 10 is enforced by class name, so wrappers of solvers with `patience` are not battery-checked beyond MultiStart(SimulatedAnnealing())); shim warnings for two-name legacy packages name both new homes; the MILP slow-tier test and benchmark use `time_limit=150`.

- 2026-09-03 Review round 2 (solvers). MILP normalises the objective so HiGHS's absolute feasibility tolerance (1e-6) cannot certify a suboptimal tour, cross-checks `lower_bound_ >= cost_ - 1e-9*max(1, |cost_|)` before setting `is_optimal_`, and on time-out returns the better of HiGHS's incumbent and the NN + 2-opt fallback; the exact kernels validate their buffers. `Insertion(strategy="cheapest")` honours the documented first-edge tie rule; ClarkeWright checks a merge on the trip oriented as it will be emitted (endpoint nearer to the depot first), summed like the greedy decoder, and may emit the reverse orientation when only that one fits; the NRBS "greedy endpoint join" branch is unreachable with the union-find rule and was removed from the text; NRBS exponents must be >= 0. IteratedLocalSearch: `temperature=None` is 0.5 % of |init cost| (1.0 when zero) and the Metropolis rule accepts non-positive deltas outright. TabuSearch: tabu marks are `it + tenure + 1` (a fixed tenure is clamped to `min(tenure, n_iter, 2**31 - 1)`), and on an asymmetric matrix a 2-opt reversal marks all reversed arcs and is tabu if any added arc is tabu; SimulatedAnnealing rejects a non-finite `t0`. AntColony floors the three `1/(rho*L)` divisors like the heuristic weights (zero-cost tours keep a finite trail); Genetic builds candidate lists lazily (only for the memetic polish); the memetic descents share `skroute/metaheuristics/_memetic.pxd`. SOM floors the Gaussian width at 1e-3 so the update never underflows. Behaviour changes for a given seed relative to wave B: TabuSearch (tenure), Insertion[cheapest] on exact ties, ClarkeWright under asymmetric time matrices.
- 2026-09-04 D31 round (traces), recorded deviations from the D30/D31 text: ClarkeWright emits one step event *before* the first merge (every customer its own out-and-back trip, so a viewer sees the initial state) and one per accepted merge — `merges + 1` events, not `merges`; the step events also carry the solver-specific keys `n_trips` (ClarkeWright) and `n_edges` (NRBS); AntColony's trail cap is `min(3n, n(n-1)/2)` on a symmetric matrix and `min(3n, n(n-1))` on an asymmetric one (every arc competes); MILP emits `edge_weights` (the support's variable values, clipped to `[0, 1]`) alongside `edges`; SOM's `ring` appears on `iteration` events only (`start` has no decoded ring yet). The four construction solvers emit `start` explicitly and read the answer before their first step, so a `True` returned at `start` silences every step event (an iterative solver still runs one iteration first); check 14 gained its non-iterative half — after a `True` a non-iterative solver emits at most one more `iteration` event before `end`, and a construction solver's result is unchanged — and §6 gained the numbered item 14 with the D30/D31 wording.
...
- 2026-09-05 maps (review r5): keyword-only extras beyond the D33 signatures — `travel_time_matrix(pause=1.0, timeout=60)`, `geocode(base_url=None)`, `fetch_pois(user_agent=None)`; every provider validates `timeout`/`pause`/`user_agent` (`base_url`, `user_agent` and `pause` are OSRM-only, `timeout` is forwarded to `googlemaps.Client`; one point issues no request); a diagonal block of one point is never requested (OSRM refuses a one-coordinate table), so n=3/chunk_size=2 is 3 requests and chunk_size=1 gives 2·C(n,2); a missing OSRM `distances` table warns once (`RuntimeWarning`); `fetch_pois` returns `labels` as an object ndarray (names/addresses/tags stay lists), rounds `[timeout:]` up to a whole second and gives the socket that plus 10 s; the Nominatim throttle runs before every attempt; body-read failures are retried like `URLError`; `MapServiceError` always carries the redacted URL and the real HTTP status; `GoogleDistanceMatrix(timeout=None)` is sent to the client only when given.
- 2026-09-05 export (review r5): a plain route visiting a node twice raises `ValueError` in the three exports; `names=`/`trip_names=` given as one `str` raise `TypeError`; the JavaScript page queues Directions requests two at a time with one retry on `OVER_QUERY_LIMIT`; KML `Style` lists `IconStyle` before `LineStyle`; the legacy `google.maps.Marker` is kept and its deprecation documented.

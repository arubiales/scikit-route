...
candidate move, fine up to a few thousand nodes), while a symmetric plain TSP uses O(1) move
deltas, candidate lists and don't-look bits and scales to tens of thousands of nodes.
...
without changing the local optimum reached when the descent converges earlier.

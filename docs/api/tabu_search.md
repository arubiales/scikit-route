...
move removed for `tenure` iterations (a tabu move is still allowed when it beats the best tour
so far). With `tenure="auto"` the tenure is redrawn every iteration from
`[ceil(sqrt(n)), 2 ceil(sqrt(n))]` (Taillard's robust tabu search); `history_` records the
best cost after every iteration and the best tour is kept in its own buffer.
...
decoded trips and their fixed charges. Tabu attributes live in an `int32` `(n, n)` matrix,
which bounds the practical size at about 5 000 nodes.

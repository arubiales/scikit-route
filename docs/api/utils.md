...
fits one instance with a per-node `service_time` (the depot's included) and requires the result
to equal a fit on the time matrix with the services folded in by hand — the definition of D32 —
so a solver must read `problem.time_or_cost`, never `problem.time`, when it prices durations
during its search; check 6 exercises the `service_time` error messages, so a `fit` override
must forward it.
